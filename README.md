...
- 曲リストへの曲追加と重複チェック
- 曲の編集・非公開・再公開（IDを変えず過去カルテを維持）
- 生徒リクエストから曲登録画面への引き継ぎ
- 関西・関東をまたぐレッスン時間の重複防止
- 未回答者だけ／明日の生徒だけへの確認LINE
- 未回答者（期限24時間前から）と前日確認の自動LINE送信・二重送信防止
- 確定レッスンの変更・キャンセル、生徒LINE通知
- 月別の出欠回数・月謝・入金状況の管理
- マイカルテ上部の次回日時・次回曲・先生メモ表示
- 関西・関東それぞれのGoogleカレンダーへ確定日程を自動同期
- 日程変更・キャンセル時のGoogleカレンダー更新（手入力予定は変更しない）
- 回答状況・次回曲・今後の日程をまとめた講師ダッシュボード
- Upstash、曲リスト、LINE設定、起動維持の状態確認
- 日程調整とカルテの一括バックアップ（最大14世代）、JSONダウンロード、画面からの復元

ログインは30日間保持され、管理URLに合言葉を付ける必要はありません。従来の `?token=...` URLも互換用に残しています。

自動通知は初期状態で、未回答確認が10:00、前日確認が18:00です。講師ホームの「自動通知の設定」から地域別に時刻とON/OFFを変更できます。`/healthz` への定期アクセスで実行されるため、Renderの起動維持設定をONにして使います。
...
同じApps Script連携を使って、確定した関西・関東の日程を別々のGoogleカレンダーへ同期します。
初回同期時に `Lesson 関西 日程` と `Lesson 関東 日程` が自動作成されます。日程の確定・手動変更・
リセット時に自動反映し、失敗した場合は講師ホームの「Googleカレンダー」から安全に再同期できます。

曲リストのI列は「公開状態」です。既存行の空欄は公開として扱います。非公開にした曲は生徒・カルテの選択肢から消えますが、IDと過去の進捗記録は残るため、再公開しても同じ曲として扱われます。
...
- カレンダーから日付を選んで追加（「今後2週間の平日を一括追加」ボタンもあり）
- 時間帯をボタンで選択（`10:00-` などのプリセット、任意の時間帯も追加可能）
- 選んだ日付 × 時間帯の組み合わせで候補リストを自動生成し、件数とともにプレビュー
...
`YOUR_TOKEN` の部分は手順4で設定した `ADMIN_TOKEN` の値に置き換えてください。
