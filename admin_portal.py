...
HOME_HTML = """<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>講師ホーム</title><style>{{style}}.hero{display:flex;justify-content:space-between;align-items:end;gap:16px;margin:4px 0 20px}.hero h2{font-size:24px;margin:0 0 5px}.hero p{margin:0;color:#687078;font-size:13px}.section{margin-top:30px}.section h2{font-size:17px;margin:0 0 5px}.schedule{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.schedule-card{background:#fff;border:1px solid #dfe3e6;border-radius:13px;padding:18px}.schedule-head{display:flex;justify-content:space-between;gap:8px}.schedule-card h3{margin:0;font-size:20px}.alert,.ready{font-size:12px;font-weight:700;border-radius:999px;padding:5px 9px;white-space:nowrap}.alert{background:#fff3dd;color:#8a5900}.ready{background:#e8f4ef;color:#0f6e56}.counts{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:16px 0}.counts div{background:#f7f9f8;border-radius:8px;padding:9px}.counts b{display:block;font-size:18px}.counts span{font-size:11px;color:#687078}.buttons{display:flex;gap:8px;flex-wrap:wrap}.buttons .button{padding:9px 12px;font-size:13px}.actions-home{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.action{display:block;background:#fff;border:1px solid #dfe3e6;border-radius:11px;padding:16px;color:inherit;text-decoration:none;min-height:130px}.action:hover{border-color:#75b9a2;box-shadow:0 5px 18px rgba(20,50,35,.07)}.action h3{font-size:16px;margin:0 0 6px}.action p{font-size:12px;line-height:1.55;color:#687078;margin:0}.stats{margin-bottom:0}.stat{border-left:4px solid #b8ddd0}.upcoming{overflow:hidden;border:1px solid #dfe3e6;border-radius:11px;background:#fff}.upcoming .small-table{border:0}@media(max-width:760px){.hero{display:block}.hero .button{display:inline-block;margin-top:12px}.schedule,.actions-home{grid-template-columns:1fr}.section{margin-top:24px}}</style></head><body><header><h1>講師ホーム</h1><form method="post" action="/admin/logout"><button class="sub">ログアウト</button></form></header><main><section class="hero"><div><h2>レッスン運用</h2><p>日程の確認から生徒管理まで、今日必要な作業をここから進められます。</p></div><a class="button sub" href="/admin/maintenance">状態・バックアップ</a></section><div class="stats"><div class="stat"><b>{{dashboard.carte.students}}</b><span>カルテ生徒</span></div><div class="stat"><b>{{dashboard.carte.next_students}}</b><span>次回曲あり</span></div><div class="stat"><b>{{dashboard.carte.open_requests}}</b><span>未対応リクエスト</span></div><div class="stat"><b>{{dashboard.carte.completed}}</b><span>実施記録</span></div></div><section class="section"><h2>日程調整</h2><p class="muted">地域ごとの回答状況を確認し、必要な対応へ進みます。</p><div class="schedule">{% for name,s in dashboard.schedules.items() %}<article class="schedule-card"><div class="schedule-head"><div><span class="tag">{{s.label}}</span><h3>{{s.label}}の日程</h3></div>{% if s.nonrespondents %}<span class="alert">未回答 {{s.nonrespondents}}人</span>{% else %}<span class="ready">回答済み</span>{% endif %}</div><div class="counts"><div><b>{{s.responded}}/{{s.members}}</b><span>回答</span></div><div><b>{{s.candidate_count}}</b><span>候補</span></div><div><b>{{s.assignment_count}}</b><span>確定枠</span></div></div><div class="buttons"><a class="button" href="/{{name}}/admin/panel">日程を開く</a><a class="button sub" href="/admin/reminders/{{name}}">リマインド</a></div></article>{% endfor %}</div></section><section class="section"><h2>日々の運用</h2><div class="actions-home"><a class="action" href="/admin/lessons"><span class="tag">日程管理</span><h3>変更・キャンセル</h3><p>確定レッスンを更新し、LINEとカレンダーへ反映します。</p></a><a class="action" href="/admin/automations"><span class="tag">自動LINE</span><h3>自動通知の設定</h3><p>未回答と前日確認の送信時刻を設定します。</p></a><a class="action" href="/admin/calendar"><span class="tag">自動同期</span><h3>Googleカレンダー</h3><p>地域別カレンダーの同期状況を確認します。</p></a></div></section><section class="section"><h2>生徒・レッスン管理</h2><div class="actions-home"><a class="action" href="/kanto/admin/carte"><span class="tag">共通</span><h3>生徒カルテ</h3><p>実施状況、やりたい曲、次回曲、生徒メモを管理します。</p></a><a class="action" href="/kanto/admin/carte/next"><span class="tag">LINE</span><h3>次回レッスンまとめ</h3><p>生徒ごとの次回曲を確認してLINEに送信します。</p></a><a class="action" href="/admin/attendance"><span class="tag">共通</span><h3>出欠・月謝</h3><p>月ごとの出席回数、欠席、月謝の状況を管理します。</p></a><a class="action" href="/admin/songs"><span class="tag">曲リスト</span><h3>曲の管理</h3><p>曲の追加、編集、非公開化、リクエスト登録を行います。</p></a><a class="action" href="/admin/maintenance"><span class="tag">安全管理</span><h3>状態確認・バックアップ</h3><p>保存状態、バックアップ、復元操作を確認します。</p></a></div></section>{% if dashboard.upcoming %}<section class="section"><h2>これからのレッスン</h2><div class="upcoming"><table class="small-table"><tr><th>地域</th><th>日時</th><th>生徒</th></tr>{% for item in dashboard.upcoming %}<tr><td><span class="tag">{{item.label}}</span></td><td>{{item.day}} {{item.time}}〜{{item.end}}</td><td>{{item.name}}</td></tr>{% endfor %}</table></div></section>{% endif %}</main></body></html>"""
