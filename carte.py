...
function draw(){let query=q.value.toLowerCase(),f=filter.value,xs=materials.filter(m=>{let st=stateOf(progress[m.id]);return(m.title+' '+m.artist+' '+m.kind).toLowerCase().includes(query)&&(f==='all'||f===st)});count.textContent=xs.length+'曲';sheet.innerHTML=xs.length?`<table><thead><tr><th>曲名</th><th>授業・メモ</th></tr></thead><tbody>${xs.map(m=>{let p=progress[m.id],st=stateOf(p),d=lessonDate(p);return `<tr><td class="song"><b>${esc(m.title)}</b><span>${esc([m.artist,m.kind].filter(Boolean).join(' ／ '))}</span></td><td class="cell" onclick="openEditor(${m.id})"><div class="cell-content"><div class="status-block"><span class="${st}">${stateLabel(st)}</span>${d?`<span class="lesson-date">${jaDate(d)}</span>`:''}</div>${p?.student_note?`<span class="memo-preview" title="${esc(p.student_note)}">${esc(p.student_note)}</span>`:''}</div></td></tr>`}).join('')}</tbody></table>`:'<div class="empty">該当する曲はありません</div>'}
...
</style></head><body><header><h1>生徒カルテ管理</h1></header><main class="page"><div class="toolbar"><input id="q" placeholder="曲名・アーティストを検索"><select id="filter"><option value="all">すべての曲</option><option value="used">実施記録がある曲</option><option value="wanted">やりたい人がいる曲</option></select><span class="count" id="count">読み込み中…</span><span class="hint">セルをクリックして実施状況・授業日・メモを入力</span></div><div class="sheet" id="sheet"><div class="empty">読み込み中…</div></div></main><dialog id="editor"><form class="modal" onsubmit="save(event)"><h2 id="editSong"></h2><div class="who" id="editStudent"></div><div class="choice"><label><input type="radio" name="state" value="notdone"> 未実施</label><label><input type="radio" name="state" value="wanted"> ★ やりたい</label><label><input type="radio" name="state" value="done"> ✓ 実施済み</label></div><div class="field"><label for="lessonDate">授業日</label><input type="date" id="lessonDate"></div><div class="field"><label for="studentNote">生徒メモ</label><textarea id="studentNote" maxlength="1000" placeholder="生徒が書いたメモを確認・編集できます"></textarea></div><div class="actions"><button type="button" onclick="editor.close()">キャンセル</button><button class="save" id="saveButton">保存</button></div></form></dialog><div class="notice" id="notice">保存しました</div><script>
