...
function draw(){let query=q.value.toLowerCase(),f=filter.value,xs=materials.filter(m=>{let st=stateOf(progress[m.id]);return(m.title+' '+m.artist+' '+m.kind+' '+m.instrument).toLowerCase().includes(query)&&(f==='all'||f===st)});count.textContent=xs.length+'曲';sheet.innerHTML=xs.length?`<table><thead><tr><th>曲名</th><th>授業・メモ</th></tr></thead><tbody>${xs.map(m=>{let p=progress[m.id],st=stateOf(p),d=lessonDate(p);return `<tr><td class="song"><div class="t"><b>${esc(m.title)}</b>${m.artist?`<em>${esc(m.artist)}</em>`:''}</div><span>${esc([m.instrument,m.kind].filter(Boolean).join(' ／ '))}</span></td><td class="cell" onclick="openEditor(${m.id})"><div class="cell-content"><div class="status-block"><span class="${st}">${stateLabel(st)}</span>${d?`<span class="lesson-date">${jaDate(d)}</span>`:''}</div>${p?.student_note?`<span class="memo-preview" title="${esc(p.student_note)}">${esc(p.student_note)}</span>`:''}</div></td></tr>`}).join('')}</tbody></table>`:'<div class="empty">該当する曲はありません</div>'}
...
function draw(){let query=q.value.toLowerCase(),f=filter.value;let materials=data.materials.filter(m=>{let matches=(m.title+' '+m.artist+' '+m.kind+' '+m.instrument).toLowerCase().includes(query);let keep=f==='all'||(f==='used'&&data.students.some(s=>progress[key(s.user_id,m.id)]))||(f==='wanted'&&data.students.some(s=>stateOf(progress[key(s.user_id,m.id)])==='wanted'));return matches&&keep});count.textContent=materials.length+'曲 × '+data.students.length+'人';sheet.innerHTML=data.students.length?`<table><thead><tr><th class="song-head">曲名</th>${data.students.map(s=>`<th class="student">${esc(s.display_name||'名前未登録')}</th>`).join('')}</tr></thead><tbody>${materials.map(m=>`<tr><td class="song"><div class="t"><b>${esc(m.title)}</b>${m.artist?`<em>${esc(m.artist)}</em>`:''}</div><span>${esc([m.instrument,m.kind].filter(Boolean).join(' ／ '))}</span></td>${data.students.map(s=>cell(m,s)).join('')}</tr>`).join('')}</tbody></table>`:'<div class="empty">生徒がまだ登録されていません</div>'}
function cell(m,s){let p=progress[key(s.user_id,m.id)],st=stateOf(p),d=lessonDate(p);return `<td class="cell" onclick="openEditor('${esc(s.user_id)}',${m.id})"><div class="cell-content"><div class="status-block"><span class="${st}">${stateLabel(st)}</span>${d?`<span class="lesson-date">${jaDate(d)}</span>`:''}</div>${p?.student_note?`<span class="memo-preview" title="${esc(p.student_note)}">${esc(p.student_note)}</span>`:''}</div></td>`}
function openEditor(uid,mid){let s=data.students.find(x=>x.user_id===uid),m=data.materials.find(x=>x.id===mid),p=progress[key(uid,mid)];editing={uid,mid};editSong.textContent=m.title;editStudent.textContent=(s.display_name||'名前未登録')+'さん';document.querySelector(`input[name="state"][value="${stateOf(p)}"]`).checked=true;lessonDate.value=lessonDate(p);studentNote.value=p?.student_note||'';editor.showModal()}
async function save(e){e.preventDefault();let st=document.querySelector('input[name="state"]:checked')?.value||'notdone',done=st==='done',button=saveButton;button.disabled=true;button.textContent='保存中';let body={user_id:editing.uid,material_id:editing.mid,lesson_done:done,lesson_date:lessonDate.value,student_note:studentNote.value,status:done?'completed':(st==='wanted'?'wanted':'planned')};try{let r=await fetch('/admin/carte/progress',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok)throw Error(d.error||'保存失敗');progress[key(editing.uid,editing.mid)]=d.progress;editor.close();draw();notice.style.display='block';setTimeout(()=>notice.style.display='none',1800)}catch(err){alert(err.message)}finally{button.disabled=false;button.textContent='保存'}}
...
:root{--green:#087f5b;--amber:#a06a00;--line:#dfe3e6;--muted:#687078;--bg:#f5f7f8}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif;margin:0;background:var(--bg);color:#202428}header{height:62px;background:#fff;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:16px;padding:0 24px}header h1{font-size:20px;margin:0}header a{margin-left:auto;color:#0f6e56;font-size:13px;text-decoration:none;border-bottom:1px solid #0f6e56}.page{padding:20px 24px}.count{color:var(--muted);font-size:13px;margin:0 0 14px}.cols{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:16px}.card{background:#fff;border:1px solid var(--line);border-radius:9px;padding:16px 18px}.card h2{font-size:15px;margin:0 0 4px}.card p.note{color:var(--muted);font-size:12px;margin:0 0 14px}.row{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f0f2f3}.row:last-child{border-bottom:0}.rank{flex:0 0 22px;text-align:right;color:var(--muted);font-size:12px}.name{flex:1;min-width:0}.name b{font-size:14px}.name em{font-style:normal;color:var(--muted);font-size:11px;margin-left:6px}.name .sub{display:block;color:var(--muted);font-size:10px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bar{flex:0 0 84px;height:6px;background:#eef0f1;border-radius:3px;overflow:hidden}.bar i{display:block;height:100%}.done .bar i{background:var(--green)}.wanted .bar i{background:var(--amber)}.num{flex:0 0 44px;text-align:right;font-size:13px;font-weight:700}.done .num{color:var(--green)}.wanted .num{color:var(--amber)}.empty{color:var(--muted);font-size:13px;padding:16px 0;margin:0}
</style></head><body><header><h1>曲ランキング</h1><a href="/admin/carte">カルテに戻る</a></header><main class="page"><p class="count" id="count">読み込み中…</p><div class="cols"><section class="card done"><h2>✓ 実施済みが多い曲</h2><p class="note">レッスンで実際に扱った人数の多い順</p><div id="doneList"></div></section><section class="card wanted"><h2>★ やりたいが多い曲</h2><p class="note">希望が集まっている順。次に用意する曲の参考に</p><div id="wantedList"></div></section></div></main><script>
const esc=s=>String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const isDone=p=>p?.lesson_done===true||(!('lesson_done' in (p||{}))&&p?.status==='completed');
function render(el,map,titles){let xs=Object.entries(map).map(([id,n])=>({m:titles[id],n})).filter(x=>x.m).sort((a,b)=>b.n-a.n||a.m.title.localeCompare(b.m.title,'ja'));if(!xs.length){el.innerHTML='<p class="empty">まだありません</p>';return}let max=xs[0].n;el.innerHTML=xs.map((x,i)=>`<div class="row"><span class="rank">${i+1}</span><span class="name"><b>${esc(x.m.title)}</b>${x.m.artist?`<em>${esc(x.m.artist)}</em>`:''}<span class="sub">${esc([x.m.instrument,x.m.kind].filter(Boolean).join(' ／ '))}</span></span><span class="bar"><i style="width:${Math.round(x.n/max*100)}%"></i></span><span class="num">${x.n}人</span></div>`).join('')}
async function load(){let r=await fetch('/admin/carte/data'),d=await r.json();if(!r.ok)throw Error(d.error||'取得に失敗しました');let titles=Object.fromEntries(d.materials.map(m=>[m.id,m])),done={},wanted={};for(let p of d.progress){let id=p.material_id;if(!titles[id])continue;if(isDone(p))done[id]=(done[id]||0)+1;else if(p.status==='wanted')wanted[id]=(wanted[id]||0)+1}count.textContent='生徒 '+d.students.length+'人 ／ 曲 '+d.materials.length+'曲';render(doneList,done,titles);render(wantedList,wanted,titles)}
