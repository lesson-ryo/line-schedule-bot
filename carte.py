...
:root{--green:#087f5b;--green2:#e8f7f0;--line:#dfe3e6;--muted:#687078;--bg:#f5f7f8}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif;margin:0;background:var(--bg);color:#202428}header{height:64px;background:#fff;border-bottom:1px solid var(--line);display:flex;align-items:center;padding:0 28px;position:sticky;top:0;z-index:5}header h1{font-size:20px;margin:0}.page{max-width:1320px;margin:24px auto;padding:0 24px}.panel{background:#fff;border:1px solid var(--line);border-radius:10px;overflow:hidden}.panel-head{padding:18px 20px;border-bottom:1px solid var(--line)}h2{font-size:20px;margin:0 0 14px}.toolbar{display:flex;align-items:center;gap:10px}.toolbar input,.toolbar select{height:40px;border:1px solid #c8cdd1;border-radius:7px;background:#fff;padding:0 12px;font-size:14px}.toolbar input{width:360px}.count{margin-left:auto;color:var(--muted);font-size:13px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;white-space:nowrap}th{font-size:12px;text-align:left;color:#596168;background:#fafbfb;padding:10px 14px;border-bottom:1px solid var(--line);position:sticky;top:64px}td{padding:12px 14px;border-bottom:1px solid #edf0f1;font-size:14px}tbody tr.data-row:hover{background:#f1faf6;cursor:pointer}.name{font-weight:700;color:#173c31}.number{text-align:right;font-variant-numeric:tabular-nums}.muted{color:var(--muted);font-size:12px}.empty{text-align:center;color:var(--muted);padding:48px}.back{display:inline-block;color:var(--green);cursor:pointer;font-weight:700;margin-bottom:14px}.badge{display:inline-block;padding:4px 9px;border-radius:12px;background:#eef1f2;font-size:12px}.badge.practicing{background:#fff3cd;color:#765900}.badge.completed{background:#e6f6ed;color:#17683e}.badge.paused{background:#f0eef8;color:#55437d}.next{color:#d26700;font-weight:700}.song-title{font-weight:650}.video{color:var(--green);text-decoration:none}.edit-row td{padding:0;background:#f8faf9}.editor{padding:16px 20px 18px 50px;display:grid;grid-template-columns:180px 120px 1fr 1fr 90px;gap:10px;align-items:end}.field label{display:block;font-size:12px;color:var(--muted);margin-bottom:5px}.field input,.field select{width:100%;height:39px;padding:8px;border:1px solid #c8cdd1;border-radius:6px;background:#fff}.check{height:39px;display:flex;align-items:center;gap:6px}.save{height:39px;border:0;border-radius:6px;background:var(--green);color:#fff;font-weight:700;cursor:pointer}.save:disabled{opacity:.55}.notice{font-size:12px;color:var(--green);min-height:18px;padding:0 20px 10px;text-align:right}@media(max-width:800px){.page{padding:0 10px}.toolbar{flex-wrap:wrap}.toolbar input{width:100%}.count{margin-left:0}.editor{grid-template-columns:1fr;padding-left:16px}header{padding:0 16px}}
</style></head><body><header><h1>生徒カルテ管理</h1></header><main class="page"><div id="summary"><div class="panel"><div class="empty">読み込み中…</div></div></div><div id="detail"></div></main><script>
const token=new URLSearchParams(location.search).get('token')||'',labels={planned:'練習予定',practicing:'練習中',completed:'完了',paused:'保留'};let data,currentUid='',currentP={},openId=null;
const esc=s=>String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const date=s=>s?new Date(s).toLocaleString('ja-JP',{year:'numeric',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'}):'—';
...
function renderSummary(){currentUid='';detail.innerHTML='';let q=(document.getElementById('sq')?.value||'').toLowerCase(),xs=data.students.filter(s=>(s.display_name||'').toLowerCase().includes(q));summary.innerHTML=`<section class="panel"><div class="panel-head"><h2>生徒一覧</h2><div class="toolbar"><input id="sq" value="${esc(q)}" placeholder="生徒名を検索" oninput="renderSummary()"><span class="count">${xs.length}人（全${data.students.length}人）／教材 ${data.materials.length}曲</span></div></div><div class="table-wrap"><table><thead><tr><th>生徒名</th><th class="number">次回</th><th class="number">練習中</th><th class="number">完了</th><th>最終更新</th></tr></thead><tbody>${xs.map(s=>`<tr class="data-row" onclick="openStudent('${esc(s.user_id)}')"><td class="name">${esc(s.display_name||'名前未登録')}</td><td class="number next">${s.next_count||0}</td><td class="number">${s.practicing_count||0}</td><td class="number">${s.completed_count||0}</td><td class="muted">${date(s.updated_at)}</td></tr>`).join('')||'<tr><td colspan="5" class="empty">該当する生徒はいません</td></tr>'}</tbody></table></div></section>`}
function openStudent(uid){let s=data.students.find(x=>x.user_id===uid);currentUid=uid;currentP=Object.fromEntries(data.progress.filter(x=>x.user_id===uid).map(x=>[x.material_id,x]));openId=null;summary.innerHTML='<span class="back" onclick="renderSummary()">← 生徒一覧へ戻る</span>';detail.innerHTML=`<section class="panel"><div class="panel-head"><h2>${esc(s.display_name||'名前未登録')}さん</h2><div class="toolbar"><input id="mq" placeholder="曲名・アーティストを検索" oninput="drawSongs()"><select id="mf" onchange="drawSongs()"><option value="active">登録曲</option><option value="all">全教材</option><option value="next">次回レッスン</option><option value="practicing">練習中</option><option value="planned">練習予定</option><option value="completed">完了</option><option value="paused">保留</option></select><span class="count" id="songCount"></span></div></div><div id="songs"></div><div class="notice" id="notice"></div></section>`;drawSongs()}
function editHtml(m,p){return `<tr class="edit-row"><td colspan="6"><div class="editor"><div class="field"><label>状態</label><select id="as${m.id}">${Object.entries(labels).map(([k,v])=>`<option value="${k}" ${(p.status||'planned')===k?'selected':''}>${v}</option>`).join('')}</select></div><label class="check"><input type="checkbox" id="an${m.id}" ${p.next_lesson?'checked':''}> 次回レッスン</label><div class="field"><label>生徒メモ</label><input id="ast${m.id}" value="${esc(p.student_note)}" placeholder="生徒と共有するメモ"></div><div class="field"><label>講師メモ</label><input id="at${m.id}" value="${esc(p.teacher_note)}" placeholder="講師用メモ"></div><button class="save" id="save${m.id}" onclick="event.stopPropagation();save(${m.id})">保存</button></div></td></tr>`}
function drawSongs(){let q=(document.getElementById('mq')?.value||'').toLowerCase(),f=document.getElementById('mf')?.value||'active';let xs=data.materials.filter(m=>{let p=currentP[m.id];if(f==='active'&&!p)return false;if(f==='next'&&!p?.next_lesson)return false;if(!['active','all','next'].includes(f)&&p?.status!==f)return false;return(m.title+' '+m.artist+' '+m.kind).toLowerCase().includes(q)});songCount.textContent=xs.length+'曲';songs.innerHTML=`<div class="table-wrap"><table><thead><tr><th style="width:70px">No.</th><th>曲名</th><th>アーティスト</th><th>形態</th><th>状態</th><th>動画</th></tr></thead><tbody>${xs.map(m=>{let p=currentP[m.id]||{},status=p.status||'planned';return `<tr class="data-row" onclick="toggleEdit(${m.id})"><td class="muted">${m.id}</td><td><span class="song-title">${p.next_lesson?'<span class="next">★ </span>':''}${esc(m.title)}</span></td><td>${esc(m.artist||'—')}</td><td class="muted">${esc(m.kind||'—')}</td><td><span class="badge ${status}">${labels[status]}</span></td><td>${m.video?`<a class="video" target="_blank" onclick="event.stopPropagation()" href="${esc(m.video.split(/\s/).find(x=>x.startsWith('http'))||'#')}">開く ↗</a>`:'—'}</td></tr>${openId===m.id?editHtml(m,p):''}`}).join('')||'<tr><td colspan="6" class="empty">該当する曲はありません</td></tr>'}</tbody></table></div>`}
function toggleEdit(id){openId=openId===id?null:id;drawSongs();if(openId)setTimeout(()=>document.getElementById('as'+id)?.focus(),0)}
async function save(id){let button=document.getElementById('save'+id);button.disabled=true;button.textContent='保存中';let body={user_id:currentUid,material_id:id,status:document.getElementById('as'+id).value,next_lesson:document.getElementById('an'+id).checked,student_note:document.getElementById('ast'+id).value,teacher_note:document.getElementById('at'+id).value};try{let r=await fetch('/admin/carte/progress?token='+encodeURIComponent(token),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),d=await r.json();if(!r.ok)throw Error(d.error||'保存失敗');currentP[id]=d.progress;data.progress=data.progress.filter(x=>!(x.user_id===currentUid&&x.material_id===id));data.progress.push(d.progress);openId=null;drawSongs();notice.textContent='保存しました';setTimeout(()=>{if(window.notice)notice.textContent=''},2500)}catch(e){button.disabled=false;button.textContent='保存';alert(e.message)}}
getData().catch(e=>summary.innerHTML='<section class="panel"><div class="empty">'+esc(e.message)+'</div></section>');
</script></body></html>"""
