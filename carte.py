...
function cell(m,s){let p=progress[key(s.user_id,m.id)],done=isDone(p),d=lessonDate(p);return `<td class="cell" onclick="openEditor('${esc(s.user_id)}',${m.id})"><span class="${done?'done':'notdone'}">${done?'✓ 実施済み':'未実施'}</span>${d?`<span class="lesson-date">${jaDate(d)}</span>`:''}${p?.student_note?'<span class="memo-mark">メモあり</span>':''}</td>`}
