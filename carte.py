...
def _student_home(user_id, materials, progress):
    """Build the compact next-lesson card shown at the top of My Carte."""
    from lesson_operations import JST, TENANT_LABELS, _slot_interval, tenant_scope

    now = datetime.now(JST)
    lessons = []
    for tenant in ("kansai", "kanto"):
        with tenant_scope(tenant):
            assignment = load_json("assignment", default=[])
        for slot in assignment if isinstance(assignment, list) else []:
            if user_id not in slot.get("member_ids", []):
                continue
    ...
    lessons.sort(key=lambda row: row["sort"])
    next_lesson = lessons[0] if lessons else None
    if next_lesson:
        next_lesson = {key: value for key, value in next_lesson.items() if key != "sort"}
    by_id = {item.get("id"): item for item in materials}
    next_songs = []
    teacher_notes = []
    for row in progress:
        if row.get("next_lesson"):
            material = by_id.get(row.get("material_id"), {})
            next_songs.append(
                {
                    "title": str(material.get("title") or "曲名未登録"),
                    "artist": str(material.get("artist") or ""),
                }
            )
            note = str(row.get("teacher_note") or "").strip()
            if note and note not in teacher_notes:
                teacher_notes.append(note)
    if not teacher_notes:
        recent = sorted(progress, key=lambda row: str(row.get("updated_at") or ""), reverse=True)
        for row in recent:
            note = str(row.get("teacher_note") or "").strip()
            if note and note not in teacher_notes:
                teacher_notes.append(note)
            if len(teacher_notes) >= 3:
                break
    return {
        "next_lesson": next_lesson,
        "next_songs": next_songs,
        "teacher_notes": teacher_notes[:3],
    }
...
function renderHome(data){let lesson=data?.next_lesson,songs=data?.next_songs||[],notes=data?.teacher_notes||[];if(!lesson&&!songs.length&&!notes.length){home.hidden=true;return}let blocks='<h2>次回レッスン</h2>';if(lesson){blocks+=`<div class="lesson">${esc(lesson.day)} ${esc(lesson.time)}〜${esc(lesson.end)}</div><div class="sub">${esc([lesson.label,lesson.location].filter(Boolean).join(' ／ '))}</div>`}else{blocks+='<div class="sub">確定した次回日程はまだありません</div>'}if(songs.length)blocks+=`<div class="block"><b>次回やる曲</b>${songs.map(x=>esc(x.title)+(x.artist?' / '+esc(x.artist):'')).join('<br>')}</div>`;if(notes.length)blocks+=`<div class="block"><b>先生からのメモ</b>${notes.map(esc).join('<br>')}</div>`;home.innerHTML=blocks;home.hidden=false}
...
def create_carte_blueprint(
    ...
    def my_carte():
        ...
        return {
            ...
            "home": _student_home(user_id, materials, progress),
        }
