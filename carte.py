...
function draw(){let query=q.value.toLowerCase(),f=filter.value;let xs=materials.filter(m=>{let st=stateOf(progress[m.id]);if(ins&&m.instrument!==ins)return false;if(!(m.title+' '+m.artist+' '+m.kind+' '+m.instrument+' '+(m.genre||'')).toLowerCase().includes(query))return false;if(f==='all')return true;if(f==='next')return !!progress[m.id]?.next_lesson;if(f==='popular')return (popular[m.id]?.wanted||0)>0;return f===st});
...
async function main(){await liff.init({liffId:LIFF_ID});if(!liff.isLoggedIn()){liff.login();return}token=liff.getIDToken();let d=await api('/api/carte/me',{});materials=d.materials;popular=d.popular||{};requests=d.requests||[];progress=Object.fromEntries(d.progress.map(p=>[p.material_id,p]));heading.textContent=(d.display_name||'')+'さんのカルテ';
 let saved=null;try{saved=localStorage.getItem('carteIns')}catch(e){}
 setIns(saved!==null?saved:(d.instrument||''),false);
 insBar.querySelectorAll('button').forEach(b=>b.onclick=()=>setIns(b.dataset.ins,true));
 draw();q.oninput=draw;filter.onchange=draw;editor.addEventListener('click',e=>{if(e.target===editor)editor.close()});reqDialog.addEventListener('click',e=>{if(e.target===reqDialog)reqDialog.close()})}main().catch(e=>sheet.innerHTML='<div class="empty">'+esc(e.message)+'</div>');
</script></body></html>"""
