...
function draw(){let query=q.value.toLowerCase(),f=filter.value,xs=materials.filter(m=>{let done=isDone(progress[m.id]);return(m.title+' '+m.artist+' '+m.kind).toLowerCase().includes(query)&&(f==='all'||(f==='done'&&done)||(f==='notdone'&&!done))});count.textContent=xs.length+'曲';sheet.innerHTML=xs.length?`<table><thead><tr><th>曲名</th><th>授業</th></tr></thead><tbody>${xs.map(m=>{let p=progress[m.id],done=isDone(p),d=lessonDate(p);return `<tr><td class="song"><b>${esc(m.title)}</b><span>${esc([m.artist,m.kind].filter(Boolean).join(' ／ '))}</span></td><td class="cell" onclick="openEditor(${m.id})"><span class="${done?'done':'notdone'}">${done?'✓ 実施済み':'未実施'}</span>${d?`<span class="lesson-date">${jaDate(d)}</span>`:''}</td></tr>`}).join('')}</tbody></table>`:'<div class="empty">該当する曲はありません</div>'}
function openEditor(id){let m=materials.find(x=>x.id===id),p=progress[id];editingId=id;editSong.textContent=m.title;document.querySelector(`input[name="done"][value="${isDone(p)}"]`).checked=true;lessonDate.value=lessonDate(p);editor.showModal()}
